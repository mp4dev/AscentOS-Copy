...
void kmain(void)
{
	...
	serial_init(COM1); // Initialize serial port
	...
	gdt_init(); // Initialize Global Descriptor Table (GDT)
	...
	// be installed.
	idt_init(); // Initialize Interrupt Descriptor Table (IDT)
	...
	irq_init(); // Initialize Interrupt Request (IRQ)
	...
	pic_init(); // Initialize Programmable Interrupt Controller (PIC)
	...
	vm_init(); // Initialize virtual memeory
	...
	pcb *ch = create_pcb_no_insert("commhand", "system", 0); // Command handler, running at highest priority
	pcb *ip = create_pcb_no_insert("sys_idle", "system", 9); // System idle process, running at lowest priority
	insert_context(ch, commhand);
